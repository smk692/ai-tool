...
2. **Given** 스레드에 여러 대화가 누적되어 있고, **When** 새 질문이 들어오면, **Then** 최근 N개의 대화를 컨텍스트로 사용합니다.
...
- **속도 제한**: Slack API 또는 Claude API 제한에 도달 시 큐잉 및 순차 처리
- **민감 정보**: 개인정보나 민감 데이터가 포함된 질문에 대한 필터링
...
- **FR-003**: 시스템은 벡터DB에서 유사한 문서를 검색할 수 있어야 합니다.
...
- **Conversation**: 대화 컨텍스트 (스레드 ID, 이전 대화 목록, 세션 정보)
- **Feedback**: 사용자 피드백 (질문, 답변, 평가, 타임스탬프)
...
- **SC-002**: 관련 문서가 있는 질문의 90% 이상에서 정확한 답변이 제공됩니다.
- **SC-003**: 사용자 긍정 피드백 비율이 80% 이상입니다.
