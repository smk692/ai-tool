...
4. **Given** 사용자가 이미지를 첨부하여 질문하면, **When** 챗봇이 이미지를 처리하고, **Then** Claude Vision API를 통해 이미지 내용을 분석한 답변이 제공됩니다.
...
- **이미지 크기 초과**: 20MB 초과 이미지는 건너뛰고 경고 로그 기록
- **지원하지 않는 이미지 형식**: JPEG, PNG, GIF, WebP 외 형식은 건너뛰기
- **이미지 다운로드 실패**: 재시도 후 실패 시 해당 이미지 건너뛰고 텍스트만 처리
...
- **FR-013**: 시스템은 Slack 메시지에 첨부된 이미지를 다운로드할 수 있어야 합니다.
- **FR-014**: 시스템은 이미지를 Claude Vision API 형식으로 변환할 수 있어야 합니다.
- **FR-015**: 시스템은 이미지와 텍스트를 함께 분석하여 답변을 생성할 수 있어야 합니다.

### Key Entities

- **Query**: 사용자 질문 (텍스트, 사용자 ID, 채널/스레드 정보, 타임스탬프, 첨부 파일 목록)
...
- **ImageContent**: Claude Vision API용 이미지 콘텐츠 (media_type, Base64 데이터, 파일명, 크기)
